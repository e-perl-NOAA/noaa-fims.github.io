...
# Cross-platform download engine override
  if (Sys.info()[["sysname"]] == "Windows") {
    download.file(url, destfile = tmp_ics, method = "wininet", quiet = TRUE)
  } else {
    # Forces the Linux runner to use curl with arguments that bypass strict SSL handshake blocks
  ...
  if (length(event_starts) > 0) {
    for (i in seq_along(event_starts)) {
      ...
      if (length(summary_line) > 0 && length(dtstart_line) > 0) {
        ...
        # Parse all times explicitly assigning standard time zones
        clean_time <- tryCatch({
          if (grepl("Z$", raw_time)) {
            # Convert raw UTC marker string to formal POSIXct object
            t <- as.POSIXct(raw_time, format="%Y%m%dT%H%M%SZ", tz="UTC")
            # Force conversion attribute to matching Eastern standard
  ...
  # FIX: Normalize the comparison clock strictly to New York time 
  # This stops the UTC GitHub Action runner from thinking today's events are in the past
  current_str <- format(Sys.time(), tz="America/New_York", usetz=FALSE)
  current_time_est <- as.POSIXct(current_str, tz="America/New_York")
  
  # Filter events using the corrected Eastern baseline clock
  ...
  # Universal fallback card if no meetings match or network drops out
