...
tryCatch({
  # FIX: Download the file to a temporary location first to bypass HTTPS blocking
  tmp_ics <- tempfile(fileext = ".ics")
  download.file(url, destfile = tmp_ics, quiet = TRUE)
  
  # Parse the downloaded local file
  events <- ical::ical_parse(tmp_ics)
  ...
  if(nrow(next_event) > 0) {
    ...
    cat('<div class="fims-meeting-card"><div class="fims-meeting-content"><h3>Next Meeting</h3><p class="mb-4 opacity-75">No upcoming meetings scheduled.</p></div></div>')
  }
}, error = function(e) {
  # Added the error message inside an HTML comment so you can inspect the webpage to see exactly why it failed!
  cat(paste0(''))
  cat('<div class="fims-meeting-card"><div class="fims-meeting-content"><h3>Next Meeting</h3><p class="mb-4 opacity-75">Unable to load calendar data.</p></div></div>')
})
```
