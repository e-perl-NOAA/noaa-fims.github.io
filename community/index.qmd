...
tryCatch({
    ...
    for (i in seq_along(event_starts)) {
      ...
      if (length(summary_line) > 0 && length(dtstart_line) > 0) {
        ...
        if (length(desc_line) > 0) {
            ...
            # 1. Aggressively chop off Google Meet and Zoom boilerplate and EVERYTHING after it
            desc_text <- sub("\\\\nGoogle Meet joining info.*", "", desc_text)
            desc_text <- sub("Google Meet joining info.*", "", desc_text)
            desc_text <- sub("\\\\nVideo call link:.*", "", desc_text)
            desc_text <- sub("\\\\nJoin Zoom Meeting.*", "", desc_text)
            
            # 2. Chop off Google's weird visual divider (if present)
            desc_text <- sub("-::~:~::~:~:~.*", "", desc_text)
            
            # 3. Convert remaining iCal newlines to HTML breaks
            desc_text <- gsub("\\\\n|\\\\N", "<br>", desc_text)
            
            # 4. Clean up trailing breaks that might be left over from the chop
            desc_text <- sub("(<br>)+$", "", desc_text)
            
            # 5. Fix escaped commas and semicolons
  ...
  if (length(future_events) > 0) {
    ...
    # Format description HTML (Now set to 5 lines max)
    desc_html <- ""
    if (nchar(desc_txt) > 0) {
      desc_html <- paste0('<p class="mb-4 small opacity-75" style="display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden;">', desc_txt, '</p>')
    } else {
