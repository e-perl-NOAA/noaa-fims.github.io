...
# Automated package loader for headless GitHub Action environments
required_packages <- c("jsonlite", "glue")
new_packages <- required_packages[!(required_packages %in% installed.packages()[,"Package"])]
if(length(new_packages)) install.packages(new_packages, repos = "[https://cloud.r-project.org](https://cloud.r-project.org)")

library(jsonlite)
library(glue)

cal_id <- "c_6eluv1h8o830k7vt8ctcs6ghto@group.calendar.google.com"
url <- paste0("[https://agenda-io.vercel.app/api/calendar/](https://agenda-io.vercel.app/api/calendar/)", cal_id)

tryCatch({
  tmp_json <- tempfile(fileext = ".json")
  
  # FORCED METHOD: Hardcoded 'wget' forces the Ubuntu runner to use its native network utility
  download.file(url, destfile = tmp_json, method = "wget", quiet = TRUE)
  
  data <- jsonlite::read_json(tmp_json)
  
  if (length(data) > 0 && !is.null(data[[1]]$summary)) {
    next_event <- data[[1]]
    
    start_time <- as.POSIXct(next_event$start, format="%Y-%m-%dT%H:%M:%S", tz="UTC")
    start_est  <- format(start_time, "%H:%M EST", tz="America/New_York")
    date_est   <- format(start_time, "%B %d, %Y", tz="America/New_York")
    ...
    <div class="fims-meeting-card">
      <div class="fims-meeting-content">
        ...
        <div class="fims-meeting-datetime">
          ...
          <div class="fims-meeting-row">
            <i class="bi bi-clock-fill fims-icon-danger"></i> <strong>{start_est}</strong>
          </div>
        </div>
      </div>
    ...
    stop("No events found in payload")
  }
}, error = function(e) {
  # Safe, styled fallback card if the GitHub runner fails to pull data
  cat('
  <div class="fims-meeting-card">
    <div class="fims-meeting-content">
      <h3 class="no-anchor">Next Meeting</h3>
