...
# Calculates today's date formatted as a URL-safe text timestamp (e.g., "2026-05-19T00:00:00Z")
...
tryCatch({
  ...
  if (length(event_starts) > 0) {
    for (i in seq_along(event_starts)) {
      ...
      if (length(summary_line) > 0 && length(dtstart_line) > 0) {
        ...
        # Clean up the description (convert iCal newlines to HTML breaks, fix commas)
        desc_text <- ""
  ...
  if (length(future_events) > 0) {
    ...
    # Format description HTML (uses CSS line-clamp to prevent massive descriptions from breaking the card)
    desc_html <- ""
    if (nchar(desc_txt) > 0) {
      desc_html <- paste0('<p class="mb-4 small opacity-75" style="display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">', desc_txt, '</p>')
    } else {
      desc_html <- '<div class="mb-4"></div>' # Keeps spacing consistent if there is no description
    }
