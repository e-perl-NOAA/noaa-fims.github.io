...
# 1. FIXED: Removed the markdown brackets from the URL
url <- "[https://calendar.google.com/calendar/ical/c_6eluv1h8o830k7vt8ctcs6ghto%40group.calendar.google.com/public/basic.ics](https://calendar.google.com/calendar/ical/c_6eluv1h8o830k7vt8ctcs6ghto%40group.calendar.google.com/public/basic.ics)"

tryCatch({
  # Download and parse the calendar
  events <- ical::ical_parse(url)
  
  # Filter for future events, convert to Eastern Time, and find the very next one
  next_event <- events %>%
    mutate(dtstart_est = with_tz(dtstart, "America/New_York")) %>%
    filter(dtstart_est > Sys.time()) %>%
    arrange(dtstart_est) %>%
    slice(1)
    
  if(nrow(next_event) > 0) {
    # Format the date (e.g., "October 24, 2024") and time (e.g., "14:00 EDT")
    date_str <- format(next_event$dtstart_est, "%B %d, %Y")
    time_str <- format(next_event$dtstart_est, "%H:%M %Z")
    summary_text <- next_event$summary
    
    # Construct the exact HTML you styled earlier
    # 2. FIXED: Swapped the <button> for an <a> tag to avoid quotation mark collisions
    html_output <- glue::glue('
    <div class="fims-meeting-card">
      <div class="fims-meeting-content">
        <h3>Next Meeting</h3>
        <p class="mb-4 opacity-75">{summary_text}</p>
        <div class="fims-meeting-datetime">
          <div class="fims-meeting-row">
            <i class="bi bi-calendar-event-fill fims-icon-danger"></i> <strong>{date_str}</strong>
          </div>
          <div class="fims-meeting-row">
            <i class="bi bi-clock-fill fims-icon-danger"></i> <strong>{time_str}</strong>
          </div>
        </div>
      </div>
    ...
    # Print the HTML to the Quarto document
    cat(html_output)
    
  } else {
    cat('<div class="fims-meeting-card"><div class="fims-meeting-content"><h3>Next Meeting</h3><p>No upcoming meetings scheduled.</p></div></div>')
  }
}, error = function(e) {
  # Fallback just in case the Google Calendar link goes down during a build
  cat('<div class="fims-meeting-card"><div class="fims-meeting-content"><h3>Next Meeting</h3><p>Unable to load calendar data.</p></div></div>')
})
```
